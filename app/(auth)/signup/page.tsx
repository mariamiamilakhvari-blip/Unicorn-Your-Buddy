...
export default function SignupPage() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    e.preventDefault()
    setError('')
    if (form.password !== form.confirm) { setError(t('signupErrorMatch')); return }
    if (form.password.length < 8) { setError(t('signupErrorLength')); return }
    setLoading(true)
    try {
      ...
      if (!res.ok) { setError(data.error ?? t('signupErrorFail')); setLoading(false); return }
      ...
      <CardContent className="pt-6 px-6 pb-8">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="space-y-1.5">
            ...
            <div className="relative">
              <Input id="password" type={showPw ? 'text' : 'password'} placeholder={t('signupPasswordHint')} value={form.password} onChange={e => set('password', e.target.value)} required className="pr-10" />
          ...
          <div className="space-y-1.5">
            <Label htmlFor="confirm">{t('signupConfirm')}</Label>
            <Input id="confirm" type={showPw ? 'text' : 'password'} placeholder={t('signupConfirmHint')} value={form.confirm} onChange={e => set('confirm', e.target.value)} required />
          </div>
        ...
        <div className="relative my-5">
          ...
          <div className="relative flex justify-center text-xs uppercase">
            <span className="bg-background px-2 text-muted-foreground">{t('signupOrWith')}</span>
          </div>
        </div>
        ...
        <p className="text-center text-sm text-muted-foreground mt-6">
          {t('signupHaveAccount')}{' '}
          <Link href="/login" className="font-semibold text-velvet-500 hover:underline">{t('signupSignIn')}</Link>
        </p>
      </CardContent>
    </Card>
  )
